    ...
    for (size_t s = 0; s < seq_len; s++) {
        ...
        for (size_t h = 0; h < n_heads; h++) {
            ...
            for (size_t j = 0; j < half_dim; j++) {
                // 【修改 1】使用 double 进行中间计算以提高精度
                // 【修改 2】逻辑改为 "除法" 以匹配 PyTorch 行为: pos / (theta ^ (2j/d))
                double freq_exponent = 2.0 * static_cast<double>(j) / static_cast<double>(head_dim);
                double theta_pow = std::pow(static_cast<double>(theta), freq_exponent);
                double freq = static_cast<double>(pos) / theta_pow;
                
                // 计算 sin/cos (使用 double 精度)
                double cos_val_d = std::cos(freq);
                double sin_val_d = std::sin(freq);
