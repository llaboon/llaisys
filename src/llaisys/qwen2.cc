...
#include "../ops/op.hpp" // 引用算子
#include "../utils.hpp"  // 引用 utils::cast (注意路径变成了 ../utils.hpp)
#include <vector>
#include <iostream>
#include <cstring>
#include <cmath>
...
// 注意：这里需要根据你的平台定义导出宏
// 如果你的项目已经包含导出宏定义（如 tensor.h 中），请使用它
// 如果没有，直接 extern "C" 在 Linux 下通常也是可见的 (除非设置了 -fvisibility=hidden)
// 为了保险，你可能需要添加 __attribute__((visibility("default")))
// 或者如果在 Windows 下 __declspec(dllexport)
